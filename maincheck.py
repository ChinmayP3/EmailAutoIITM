...
def getHtmlContent2(name,company):
    ...
    return html_content
first = True
attached_file = None  # Keep track of the attached file

# Upload the file only once outside the loop
if args.pdf:
    pdf_file_path = args.pdf
    with open(pdf_file_path, 'rb') as pdf_file:
        attached_file = MIMEBase('application', 'octet-stream')
        attached_file.set_payload(pdf_file.read())
        encoders.encode_base64(attached_file)
        attached_file.add_header('Content-Disposition', f'attachment; filename="{pdf_file_path}"')
...
with open(args.file, mode='r') as file:
    ...
    for lines in csvFile:
        ...
        try:
            msg = MIMEMultipart()
            if attached_file:
                msg.attach(attached_file)  # Reuse the attached file for each email
            msg['From'] = "CSE Placement Team IIT Madras<"+email_address+">"
            msg['Subject'] = 'Greetings from IIT Madras - Internship Season 2023-24'
            msg['To'] = mail
